...
def preProcess():
    path = 'data/'
    df_train = pd.read_csv(path + 'train.csv') #（1600，40）
    df_test = pd.read_csv(path + 'test.csv')   #（400，40）
    df_train.drop(['Id'], axis = 1, inplace = True)
    df_test.drop(['Id'], axis = 1, inplace = True)
    df_test['Label'] = -1
    data = pd.concat([df_train, df_test])   # 合并（2000，40）
    data = data.fillna(-1) # 填充空值
...
def gbdt_lr_predict(data, category_feature, continuous_feature):
    ...
    for col in category_feature:
        ...
        data = pd.concat([data, onehot_feats], axis=1) # 共（1999，13105）
