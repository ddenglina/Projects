...
def segment_signal_without_transition(data,label,label_index,window_size):# ((7680,32),(40),40,128)
...
def apply_mixup(dataset_file,window_size,label): # initial empty label arrays
    ...
    data_in = data_file_in["data"].transpose(0,2,1)  # (40,40,8064)-->(40MV,8064,40 Channel)-->(40mv,60s,32*128)
    ...
    if label=="arousal":
        ...
        label=2
    label_in= data_file_in["labels"][:,label]>5 # True or False
    label_inter	= np.empty([0]) # initial empty data arrays  # (0,)
    data_inter_cnn	= np.empty([0,window_size, 9, 9])   # (0,128,9,9)
    data_inter_rnn = np.empty([0, window_size, 32])     # (0,128,32)
    trials = data_in.shape[0] # 40

    # Data pre-processing
    for trial in range(0,trials): # 基线求均值
        base_signal = (data_in[trial,0:128,0:32]+data_in[trial,128:256,0:32]+data_in[trial,256:384,0:32])/3
        data = data_in[trial,384:8064,0:32]  # (7680,32)
        # compute the deviation between baseline signals and experimental signals
        for i in range(0,60): # 60s数据减去基线值
            data[i*128:(i+1)*128,0:32]=data[i*128:(i+1)*128,0:32]-base_signal
        label_index = trial # 40
        #read data and label
        data = norm_dataset(data)    # ((7680,32),(40),40,128)
        data, label = segment_signal_without_transition(data, label_in,label_index,window_size) # 分为1s一段  (7680,32),(60,)
        # cnn data process
        data_cnn = dataset_1Dto2D(data) #(60,128,9,9)
        data_cnn = data_cnn.reshape ( int(data_cnn.shape[0]/window_size), window_size, 9, 9) # (60,128,9,9)
        # rnn data process
        data_rnn = data.reshape(int(data.shape[0]/window_size),window_size,32)  # (60,128,32)
        # append new data and label
        data_inter_cnn  = np.vstack([data_inter_cnn, data_cnn]) # 垂直堆叠,(2400,128,9,9)
        data_inter_rnn = np.vstack([data_inter_rnn, data_rnn]) # 垂直堆叠,(2400,128,32)
        label_inter = np.append(label_inter, label) # (2400,)
...
if __name__ == '__main__' :
    ...
    for record in record_list:
        ...
        output_data_rnn = output_dir + record + "_rnn_dataset.pkl"
