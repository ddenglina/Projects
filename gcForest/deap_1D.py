...
for data_file in file_list:
    ...
    for curr_fold in range(fold):
        # 十折交叉验证 分数据
        fold_size = X_rnn.shape[0] // fold  # 2400//10=240
        indexes_list = [i for i in range(len(X_rnn))] # [0,2400]
        # indexes = np.array(indexes_list)

        split_list = [i for i in range(curr_fold * fold_size, (curr_fold + 1) * fold_size)]  # [0,240],[240,240*2],....
        ...
        _ = gcf.cascade_forest(X_rnn_tr_mgs, y_tr)                        # 使用多粒度扫描的输出作为级联结构的输入，这里要注意
        pred_proba = np.mean(gcf.cascade_forest(X_rnn_te_mgs), axis=0)    # 级联结构不能直接返回预测，而是最后一层级联Level的结果
    ...
    print('mean_accuracy / fold:',mean_accuracy / fold)
